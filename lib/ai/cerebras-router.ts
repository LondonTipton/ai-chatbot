...
export function shouldUseTavilyForSimpleQuery(query: string): boolean {
  ...
  const requiresSearch = CURRENT_INFO_PATTERNS.some(pattern => pattern.test(lowerQuery));

  if (requiresSearch) {
    console.log("[Cerebras Router] 🔍 Query requires Tavily QNA (current/specific info needed)", {
  ...
  const isDefinitional = DEFINITIONAL_PATTERNS.some(pattern => pattern.test(lowerQuery));

  if (isDefinitional && !requiresSearch) {
    console.log("[Cerebras Router] ⚡ Definitional query: Cerebras direct (100-500ms)", {
      query: query.substring(0, 80),
  ...
  console.log("[Cerebras Router] ⚡ General query: Cerebras direct (default fast path)", {
    query: query.substring(0, 80),
  });
  return false;
}
...
export function needsWebTools(query: string): boolean {
  ...
  const needsTools = TOOL_INDICATORS.some(pattern => pattern.test(lowerQuery));
...
export function getOptimalRoute(
  ...
  if (complexity === "simple" || complexity === "light") {
    ...
    if (shouldUseTavilyForSimpleQuery(query)) {
      console.log("[Cerebras Router] 🎯 Route: Tavily QNA (simple + search needed)");
      return "tavily-qna";
    }
    
    console.log("[Cerebras Router] 🎯 Route: Cerebras direct (simple + no search)");
    return "cerebras-direct";
  }

  // Medium and above use full workflow
  console.log("[Cerebras Router] 🎯 Route: Full workflow (medium/deep complexity)");
