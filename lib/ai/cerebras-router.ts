...
const logger = createLogger("ai/cerebras-router");
...
export function needsWebTools(query: string): boolean {
  ...
  logger.log("[Cerebras Router] 🔧 Tool requirement check", {
    query: query.substring(0, 80),
    needsTools,
  });
...
export function logRoutingDecision(
  ...
  logger.log("[Cerebras Router] 🎯 ROUTING DECISION");
  logger.log("=".repeat(80));
  logger.log(
    `Query: "${query.substring(0, 100)}${query.length > 100 ? "..." : ""}"`
  );
  logger.log(`Complexity: ${complexity}`);
  logger.log(`Route: ${route}`);
  logger.log(`Expected Latency: ${expectedLatency}`);
  logger.log("=".repeat(80));
}
